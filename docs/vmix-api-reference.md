...
- Файлы `logo_a.png` и `logo_b.png` сохраняются автоматически в папку logos при сохранении матча

**Пример полного запроса**:

```
GET http://localhost:8088/api?Function=SetImage&Input=Input1&SelectedName=TeamALogo&Value=http%3A%2F%2F192.168.144.100%3A3000%2Flogos%2Flogo_a.png
```

**Примечание**: В примере используется `SetImage` (а не `SetText`) для установки изображения. IP адрес `192.168.144.100` соответствует выбранному сетевому интерфейсу в подсети vMix.
