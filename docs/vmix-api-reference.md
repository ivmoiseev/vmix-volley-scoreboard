# Справочник HTTP API vMix для работы с текстовыми полями

## Обзор

Этот документ описывает HTTP API запросы vMix для изменения текстовой информации в инпутах. vMix предоставляет HTTP API для управления инпутами через GET запросы.
...
- `[IP_сервера]` - IP адрес компьютера в локальной сети (определяется автоматически, игнорирует VPN)
- `[порт]` - порт мобильного HTTP сервера (по умолчанию 3000)
- Путь `/logos/` обслуживается мобильным сервером как статические файлы
- Файлы `logo_a.png` и `logo_b.png` сохраняются автоматически в папку `logos/` проекта
...
**Оригинальное значение**: `http://192.168.1.100:3000/logos/logo_a.png`

**Закодированное значение**: `http%3A%2F%2F192.168.1.100%3A3000%2Flogos%2Flogo_a.png`
