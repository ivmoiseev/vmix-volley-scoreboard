...
│                                                 │
│  Настройка инпутов:                             │
│  Заявка:              [Input1]  [Оверлей 1 ▼]   │
│  Статистика:          [Input2]  [Оверлей 4 ▼]   │
│  Состав команды:      [Input3]  [Оверлей 1 ▼]   │
│  Стартовый состав:    [Input4]  [Оверлей 1 ▼]   │
│  Текущий счет:        [Input5]  [Оверлей 1 ▼]   │
│  Счет после П1:       [Input6]  [Оверлей 1 ▼]   │
│  Счет после П2:       [Input7]  [Оверлей 1 ▼]   │
│  Счет после П3:       [Input8]  [Оверлей 1 ▼]   │
│  Счет после П4:       [Input9]  [Оверлей 1 ▼]   │
│  Счет после П5:       [Input10] [Оверлей 1 ▼]   │
│  Плашка судья 1:      [Input11] [Оверлей 1 ▼]   │
│  Плашка судья 2:      [Input12] [Оверлей 1 ▼]   │
│                                                 │
│  [Сохранить настройки]                          │
└─────────────────────────────────────────────────┘
```
...
- Выбор оверлея (1-8) для каждого инпута отдельно через выпадающий список
- Сохранение настроек в глобальный файл `settings.json`
