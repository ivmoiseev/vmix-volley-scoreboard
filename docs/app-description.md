...
- **Express** - HTTP сервер для мобильного доступа
- **Canvas API** - обработка изображений (логотипы, изменение размера)
...
2. **При первом запуске настраивает подключение к vMix (адрес, инпуты, оверлеи для каждого инпута) - настройки сохраняются автоматически**
3. Создает новый матч или открывает существующий
4. Настраивает параметры матча (команды, турнир, место, дата/время, логотипы, город)
5. **При первом использовании запускает мобильный сервер и генерирует уникальную ссылку - ссылка сохраняется автоматически**
...
11. **При следующем запуске приложения все настройки (vMix, мобильный сервер, ссылка) восстанавливаются автоматически**
