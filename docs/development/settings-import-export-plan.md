...
### Особенность production: «старые» настройки и экспорт

Экспорт настроек (через меню приложения) перед записью **валидирует текущий** `settings.json`. Если в `userData/settings.json` осталась “легаси” конфигурация, нарушающая новые правила (например, один и тот же `autoEventType` выбран у нескольких vMix-инпутов), экспорт будет блокироваться ошибкой «Настройки невалидны».

Чтобы это не ломало работу в production после обновлений, при загрузке настроек добавлена автонормализация секции `vmix`:
- оставляет только один `isScoreInput`;
- гарантирует «один тип события = один инпут» для `autoEventTypes`, удаляя дубликаты (тип остаётся за первым инпутом по `inputOrder`).
