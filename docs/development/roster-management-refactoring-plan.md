...
- **Структура сверху вниз:**
  ...
  3. **Состав команды** — таблица и кнопки (Импорт, Экспорт, Добавить игрока) — без изменений по расположению.
...
1. **Общие компоненты (без дублирования кода):**
   ...
   - **TeamColorsEditor** (`src/renderer/components/TeamColorsEditor.tsx`) — два поля: цвет формы + цвет либеро (пикер + текст). Используется на обеих страницах.
