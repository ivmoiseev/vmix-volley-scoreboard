...
**Цель:** Подробное описание всей логики для анализа и рефакторинга
...
- Обычные партии (1-4): минимум 25 очков с разницей 2
- 5-я партия: минимум 15 очков с разницей 2
- При счете 24:24 (14:14) игра продолжается до разницы в 2

#### `getSetWinner(scoreA, scoreB)`
- Определяет победителя партии
- Возвращает `'A'`, `'B'` или `null`

#### Текущая реализация: волейбол в зале (ВФВ)
- Реализация рассчитана на **классический волейбол в зале** по правилам ВФВ/ФИВБ.
- Для поддержки **пляжного** и **снежного** волейбола в России потребуется параметризация правил (пороги очков, количество партий, сетбол/матчбол). Подробное описание вариантов и рекомендации по адаптации: [Правила вариантов волейбола (зал, пляж, снежный)](./volleyball-variants-rules.md).
