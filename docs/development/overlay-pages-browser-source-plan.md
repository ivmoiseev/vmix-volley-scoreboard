...
- Ниже — таблица/список: **номер и имя** (позиции игроков не отображаем — часто остаются незаполненными). Стартовый состав выделен (например звёздочка или стиль).
...
- **Из матча:** `tournament`, `tournamentSubtitle`, `venue`, `location`, `date`, `time`, `timezone`, `teamA.name`, `teamB.name`, а также **логотипы команд** (URL из `logoPath` через API).
- **Отображение:** название турнира, место, дата/время; **названия команд с логотипами рядом** (логотипы достаточно крупные для титра).

### 6.3 Rosters

- **Из матча:** `teamA.name`, `teamB.name`, **логотипы команд** (URL через API), `teamA.roster[]`, `teamB.roster[]` (номер, имя, стартовый состав; позицию не показываем — часто незаполнена), при желании `teamA.coach`, `teamB.coach`.
- **Отображение:** две колонки или два блока (команда A / команда B); у каждого блока — **название команды и логотип рядом**; ниже — списки игроков (номер и имя), выделение стартового состава.
...
- [ ] Реализован `GET /api/overlay/match` с лёгким JSON, URL логотипов, цветами команд и полями `setballTeam`/`matchballTeam`.
- [ ] Реализованы `GET /overlay/scoreboard` (цвета формы, всплывающий блок сетбол/матчбол), `GET /overlay/intro` и `GET /overlay/rosters` (с логотипами у названий команд) с HTML 1920×1080 и прозрачным фоном.
- [ ] На всех страницах данные подставляются безопасно (без XSS).
- [ ] На странице «Мобильный доступ» отображается блок «Ссылки для vMix/OBS» с копируемыми ссылками (только при запущенном сервере) — см. раздел 5.
- [ ] Обновлена пользовательская документация (и при необходимости README/development).
- [ ] Выполнена ручная проверка в браузере и в vMix или OBS.
