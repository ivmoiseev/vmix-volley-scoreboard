...
- **Команда А:**
  ...
  - Цвет формы игроков (цветовой пикер)
  - Цвет формы либеро (цветовой пикер, опционально)
...
- **Выбор сетевого интерфейса:**
  - Выпадающий список со всеми доступными сетевыми интерфейсами компьютера
  - Отображение имени интерфейса, IP адреса и типа (VPN/публичный/частный)
  - Выбранный интерфейс сохраняется в настройках (`mobile.selectedIP`)
  - Выбор доступен только при остановленном сервере (требуется перезапуск для применения изменений)
  - Автоматическое определение первого доступного интерфейса при отсутствии выбора
  - Решение проблемы работы в разных подсетях (например, сервер в 10.x.x.x, vMix в 192.168.144.x)
- Отображение статуса встроенного HTTP сервера
- Отображение IP адреса, порта и URL сервера (с учетом выбранного интерфейса)
...
- Сохранение состояния сервера, выбранного IP и ссылки в глобальных настройках
...
**Назначение:** Загрузка, редактирование и сохранение составов команд, а также внешнего вида команды (название, логотип, цвета формы)
...
│                                                 │
│  Внешний вид команды:          [Импорт] [Экспорт] │
│  Название команды: [Название]  Город: [Город команды] │
│  Логотип: [Превью] [Загрузить/Изменить] [Удалить] │
│  Цвет формы игроков: [пикер] [#hex]             │
│  Цвет формы либеро: [пикер] [#hex или пусто] [Поменять цвета] │
│                                                 │
│  Состав команды:                [Добавить игрока] │
│  №  Имя              Позиция   Стартовый        │
│  1  [Иванов И.И.]    [OH ▼]    ☑                │
│  ...                                            │
│  Тренер: [Имя тренера]                          │
│                                                 │
│  Стартовый состав (сетка, drag-and-drop):      │
│  [I] [II] [III] [IV] [V] [VI] [Либеро 1] [Либеро 2] │
│                                                 │
│  [Отменить] [Сохранить и вернуться]            │
└─────────────────────────────────────────────────┘
```

**Функциональность:**

- Выбор команды (A или B) — отображаются данные выбранной команды
- **Внешний вид команды:** в заголовке блока — кнопки «Импорт» и «Экспорт» (относятся ко всем параметрам команды: название, город, логотип, цвета, состав, тренер). Ниже: название, логотип (загрузка/удаление), цвет формы игроков и либеро, кнопка «Поменять цвета» (меняет местами значения двух цветов)
- **Состав команды:** в заголовке блока — только кнопка «Добавить игрока»
- Редактирование списка игроков (добавление, удаление, порядок drag-and-drop), позиция, стартовый состав
- Указание тренера команды
- Стартовый состав: порядок игроков в сетке (в т.ч. либеро) перетаскиванием
- **Импорт/экспорт:** один формат JSON. Обязательное поле `roster` (массив); опционально `coach`, `name`, `city`, `color`, `liberoColor`, `logoBase64`. Экспорт — для выбранной команды (имя файла: `roster_<название>_<timestamp>.json`)
...
Тема интерфейса (светлая/тёмная) переключается только через меню **Вид → Переключить тему**. Выбор сохраняется в настройках приложения. Подробнее см. [Дизайн и темы](../development/DESIGN.md).
