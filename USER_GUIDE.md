...
3. На главной странице отображается:
   - Статус подключения к vMix (если настроено)
...
Навигация между страницами осуществляется через меню в верхней части окна.
...
2. Включите переключатель "Включить статистику"
...
- Статистика автоматически отображается на соответствующих плашках vMix (если настроено)

### Управление плашками vMix

На странице "Управление матчем" есть кнопки для управления видимостью плашек в vMix:

- **Заявка** — информация о командах и турнире
- **Статистика** — расширенная статистика матча
- **Состав** — полный состав команды
- **Стартовый состав** — стартовый состав команды
- **Текущий счет** — счет текущей партии
- **Счет П1, П2, П3, П4, П5** — счет завершенных партий
- **Судья 1, Судья 2** — информация о судьях

**Использование:**
- Нажмите кнопку, чтобы показать/скрыть плашку в соответствующем оверлее vMix
- Активные плашки отмечены галочкой (✓) или подсвечены
- Состояние кнопок синхронизируется с реальным состоянием оверлеев в vMix

**Важно:** Для работы управления плашками необходимо настроить подключение к vMix (см. раздел "Настройка интеграции с vMix").
...
Страница "Настройки vMix" позволяет настроить подключение к vMix и конфигурацию инпутов.

### Настройки подключения

**Основные параметры:**
1. **IP адрес** — IP адрес компьютера с vMix (например, `192.168.1.100`)
2. **Порт** — порт HTTP API vMix (по умолчанию `8088`)

**Как узнать IP адрес компьютера с vMix:**
- На компьютере с vMix откройте командную строку
- Введите `ipconfig` (Windows) или `ifconfig` (Linux/Mac)
- Найдите IP адрес в разделе "IPv4 Address" (обычно начинается с 192.168.x.x или 10.x.x.x)

**Проверка подключения:**
1. После ввода IP и порта нажмите кнопку **"Тест подключения"**
2. Если подключение успешно, появится сообщение "Подключено"
3. Если ошибка, проверьте:
   - Правильность IP адреса и порта
   - Доступность компьютера с vMix по сети
   - Включен ли HTTP API в vMix (по умолчанию включен)

### Настройка инпутов

Приложение поддерживает настройку различных инпутов для отображения информации в vMix.
...
1. **Выберите инпут** в левой панели (вкладки инпутов)
2. **Включите инпут** — установите чекбокс "Включить инпут"
3. **Настройте общие параметры:**
   - **Имя или номер инпута** — имя или номер инпута в vMix (например, `Input5` или `5`)
   - **Номер оверлея** — выберите оверлей (1-8), в котором будет отображаться инпут
4. **Настройте поля инпута:**
   - Для каждого поля можно:
     - Включить/выключить поле (чекбокс)
     - Указать `fieldIdentifier` — кастомное имя поля для vMix (например, `TeamA`, `ScoreA`)
     - Тип поля отображается рядом с названием:
       - **text** — текстовое поле
       - **color** — поле цвета (для команды `SetColor`)
       - **visibility** — поле видимости (для показа/скрытия элементов)
       - **image** — поле изображения (для логотипов через `SetImage`)

**Пример настройки инпута "Заявка":**
- Имя инпута: `Input5`
- Оверлей: `1`
- Поля:
  - Команда А (text) — `fieldIdentifier: TeamA`
  - Команда Б (text) — `fieldIdentifier: TeamB`
  - Цвет А (color) — `fieldIdentifier: ColorA`
  - Цвет Б (color) — `fieldIdentifier: ColorB`

**Сохранение настроек:**
1. После настройки всех инпутов нажмите кнопку **"Сохранить настройки"**
2. Настройки сохранятся в файл `settings.json`
3. Настройки автоматически загрузятся при следующем запуске приложения

**Важно:**
- Настройки vMix сохраняются глобально и применяются ко всем матчам
- При изменении настроек необходимо перезагрузить матч или обновить данные в vMix (F5)
- Убедитесь, что имена инпутов и `fieldIdentifier` соответствуют настройкам в vMix
