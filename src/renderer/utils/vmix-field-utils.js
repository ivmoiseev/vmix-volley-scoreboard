/**
 * Обертка-адаптер для использования vmix-field-utils в renderer части
 * Дублируем функции здесь, чтобы избежать проблем с импортом CommonJS модулей в Vite
 * Это гарантирует, что функции всегда доступны в renderer части
 */
 ...
 */
export function getFullFieldName(fieldIdentifier, type) {
  if (!fieldIdentifier) {
    fieldIdentifier = '';
  }
 ...
 */
export function removeFieldSuffix(fieldIdentifier) {
  if (!fieldIdentifier) {
    return '';
  }
 ...
 */
export function hasFieldSuffix(fieldIdentifier, type) {
